...
Пошук тендера по ідентифікатору
  ...
  Run Keyword And Return If   "UA-AR-P" in "${auction_id}"     cuex.Пошук об’єкта МП по ідентифікатору   ${user_name}   ${auction_id}
  Run Keyword And Return If   "UA-LR-SSP" in "${auction_id}"   cuex.Пошук лоту по ідентифікатору         ${user_name}   ${auction_id}
...
Пошук тендера у разі наявності змін
  ...
  Run Keyword If   ${status}   cuex.Пошук тендера по ідентифікатору   ${user_name}   ${auction_id}
...
Завантажити документ в тендер з типом
  ...
  cuex.Пошук тендера по ідентифікатору   ${user_name}   ${auction_id}
...
Завантажити документ
  ...
  cuex.Завантажити документ в тендер з типом   ${user_name}   ${auction_id}   ${file_path}
...
Отримати інформацію із тендера
  [Arguments]   ${user_name}   ${auction_id}   ${field}
  cuex.Пошук тендера у разі наявності змін   ${TENDER['LAST_MODIFICATION_DATE']}   ${user_name}   ${auction_id}
...
Завантажити ілюстрацію
  [Arguments]   ${user_name}   ${auction_id}   ${file_path}
  cuex.Завантажити документ в тендер з типом   ${user_name}   ${auction_id}   ${file_path}   illustration
...
Оновити сторінку з об'єктом МП
  [Arguments]   ${user_name}   ${asset_id}
  cuex.Пошук об’єкта МП по ідентифікатору   ${user_name}   ${asset_id}
...
Завантажити ілюстрацію в об'єкт МП
  ...
  cuex.Завантажити документ в об'єкт МП з типом   ${user_name}   ${asset_id}   ${file_path}   illustration
...
Оновити сторінку з лотом
  ...
  cuex.Пошук лоту по ідентифікатору   ${user_name}   ${lot_id}
...
Завантажити ілюстрацію в лот
  [Arguments]   ${user_name}   ${lot_id}   ${file_path}
  cuex.Завантажити документ в лот з типом   ${user_name}   ${lot_id}   ${file_path}   illustration
...
Активувати контракт
  ...
  cuex.Пошук контракту по ідентифікатору   ${user_name}   ${contract_id}
   Wait Until Keyword Succeeds   6 x   60 s   Run Keywords
  ...   Reload Page
